...
class Team{
    ...
    {
        $sql = "SELECT * FROM mytableWHERE first_name LIKE '{$keywords}%' OR team = '{$keywords}'";
        $query = $this->ConDB->prepare($sql);
        if( $query->execute()){
            $result = $query->fetchAll(PDO::FETCH_ASSOC);
            $data = json_encode($result);
    ...
    {
        $sql = "INSERT INTO `mytable` (`identifier`, `first_name`, `last_name`, `team`, `position`, `image`, `detail_user`)";
